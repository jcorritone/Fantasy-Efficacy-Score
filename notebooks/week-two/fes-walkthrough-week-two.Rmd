---
title: "Fantasy Efficacy Score (FES): Week Two"
...
cleaned_espn_week_two_data = cleaned_espn_week_two_data %>%
  filter(points != "--")
...
write.csv(cleaned_espn_week_two_data, 
          "/Users/jcorritone/Desktop/cleaned_espn_week_two_data.csv", 
          row.names = FALSE)
```
...
```{r}
week_two_baselines = cleaned_espn_week_two_data %>%
    group_by(position) %>%
    summarise(baseline = sum(points * start_percent, na.rm = TRUE) / sum(start_percent, na.rm = TRUE),
              .groups = "drop")

fes_week_two_data = cleaned_espn_week_two_data %>%
  left_join(week_two_baselines, by = "position") %>%
  mutate(fes = (points - baseline) * start_percent)

write.csv(fes_week_two_data, 
          "/Users/jcorritone/Desktop/fes_week_two_data.csv", 
          row.names = FALSE)
```
...
# Histogram of FES
ggplot(fes_week_two_data %>% filter(start_percent > .10), aes(x = fes)) +
  geom_histogram(bins = 20, fill = "steelblue", color = "black") +
  labs(title = "Histogram of FES (Week 2)", subtitle = "Players Started in Greater Than 10% of ESPN Leagues", x = "FES", y = "Number of Players") +
  theme_linedraw()

# Scatter Plot of Fantasy Points & FES
ggplot(fes_week_two_data %>% filter(start_percent > .10), aes(x = points, y = fes)) +
  geom_point(alpha = 0.6, ) +
  geom_smooth(method = "lm", se = F, color = "darkred", alpha = 0.6) +
  labs(title = "Scatter Plot of FES & Fantasy Points (Week 2)", subtitle = "Players Started in Greater Than 10% of ESPN Leagues", x = "Fantasy Points", y = "FES") +
  theme_bw(base_size = 10, )

# Box Plot of FES Across Positions
ggplot(fes_week_two_data %>% filter(start_percent > .10), aes(x = position, y = fes, fill = position)) +
  geom_boxplot() +
  labs(title = "Box Plot of FES Across Positions (Week w)", subtitle = "Players Started in Greater Than 10% of ESPN Leagues", x = "Player Position", y = "FES") +
  theme_linedraw()

# Scatter Plot of Start Percent & FES
ggplot(fes_week_two_data, aes(x = start_percent, y = fes)) +
  geom_point(alpha = 0.6, ) +
...
```{r}
fes_week_two_data <- fes_week_two_data %>%
  mutate(highlight = ifelse(player_name %in% c("Amon-Ra St. Brown", "Derrick Henry", "Brandon Aubrey", "Jared Goff"), "Highlight", "Normal"))
library(ggrepel)

ggplot(fes_week_two_data %>% filter(start_percent > 0.10), 
       aes(x = points, y = fes)) +
  ...
  geom_text_repel(
    data = subset(fes_week_two_data, player_name %in% c("Amon-Ra St. Brown", "Derrick Henry", "Brandon Aubrey", "Jared Goff")),
  ...
  labs(title = "Scatter Plot of FES & Fantasy Points (Week 2)",
